...
static std::wstring ConvertStringToBSTR(const char *pSrc) {
  ...
  if (const auto cwch = MultiByteToWideChar(CP_ACP, 0, pSrc, -1, nullptr, 0)) {
    ret.resize(cwch - 1);
    if (!MultiByteToWideChar(CP_ACP, 0, pSrc, -1, &ret[0],
...
static std::string ConvertBSTRToString(BSTR pSrc) {
  ...
  std::string ret;
  if (const auto len =
          WideCharToMultiByte(CP_ACP, 0, pSrc, src_len + 1, NULL, 0, 0, 0)) {
    ret.resize(len + 1, '\0');
...
static std::string version_name() {
  ...
  if (FAILED(CoCreateInstance(CLSID_WbemLocator, 0, CLSCTX_INPROC_SERVER,
                              IID_IWbemLocator,
                              reinterpret_cast<void **>(&webm_loc_raw))))
    return {};
  std::unique_ptr<IWbemLocator, iware::detail::release_deleter> webm_loc(
      webm_loc_raw);

  IWbemServices *webm_services_raw;
  if (FAILED(webm_loc->ConnectServer(&ConvertStringToBSTR("ROOT\\CIMV2")[0],
                                     nullptr, nullptr, 0, 0, 0, 0,
                                     &webm_services_raw)))
    return {};
  std::unique_ptr<IWbemServices, iware::detail::release_deleter> webm_services(
      webm_services_raw);

  if (FAILED(
          CoSetProxyBlanket(webm_services.get(), RPC_C_AUTHN_WINNT,
                            RPC_C_AUTHZ_NONE, nullptr, RPC_C_AUTHN_LEVEL_CALL,
                            RPC_C_IMP_LEVEL_IMPERSONATE, nullptr, EOAC_NONE)))
    return {};

  IEnumWbemClassObject *query_iterator_raw;
  if (FAILED(webm_services->ExecQuery(
          &ConvertStringToBSTR("WQL")[0],
          &ConvertStringToBSTR("SELECT Name FROM Win32_OperatingSystem")[0],
          WBEM_FLAG_FORWARD_ONLY | WBEM_FLAG_RETURN_IMMEDIATELY, nullptr,
          &query_iterator_raw)))
    return {};
  std::unique_ptr<IEnumWbemClassObject, iware::detail::release_deleter>
      query_iterator(query_iterator_raw);
  ...
  while (query_iterator) {
    ...
    if (!iter_result)
      break;
    std::unique_ptr<IWbemClassObject, iware::detail::release_deleter> value(
        value_raw);

    VARIANT val;
    value->Get(L"Name", 0, &val, 0, 0);
    iware::detail::quickscope_wrapper val_destructor{
        [&] { VariantClear(&val); }};
